...
# Función para preguntar si se desea reiniciar el sistema
function ask_reboot() {
    if (whiptail --title "Reiniciar el sistema" --yesno "¿Quieres reiniciar el sistema ahora?" 10 60); then
        sudo reboot
    else
        echo "Recuerda reiniciar el sistema más tarde para aplicar completamente los cambios."
    fi
}
...
echo "La configuración del idioma y zona horaria se ha completado."
ask_reboot
